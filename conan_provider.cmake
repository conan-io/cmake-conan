...
function(conan_install CONANFILE)
    ...
    execute_process(COMMAND ${CONAN_COMMAND} install ${CONANFILE} ${CONAN_ARGS} ${ARGN} --format=json
                    RESULT_VARIABLE return_code
                    OUTPUT_VARIABLE conan_stdout
                    ERROR_VARIABLE conan_stderr
                    ECHO_ERROR_VARIABLE    # show the text output regardless
...
macro(conan_provide_dependency method package_name)
    ...
    if(NOT _conan_install_success)
        ...
        if(CONAN_CONANFILE_TXT_PATH AND EXISTS "${CONAN_CONANFILE_TXT_PATH}")
            file(READ "${CONAN_CONANFILE_TXT_PATH}" outfile)
            ...
            if(NOT IS_ABSOLUTE ${CONAN_CONANFILE_TXT_PATH})
              set(_conanfile_path "${CMAKE_SOURCE_DIR}/${CONAN_CONANFILE_TXT_PATH}")
            else()
              set(_conanfile_path ${CONAN_CONANFILE_TXT_PATH})
            endif()
        elseif(EXISTS "${CMAKE_SOURCE_DIR}/conanfile.py")
            ...
            set(generator "")
            set(_conanfile_path "${CMAKE_SOURCE_DIR}/conanfile.py")
        elseif (EXISTS "${CMAKE_SOURCE_DIR}/conanfile.txt")
            ...
            set(generator "-g;CMakeDeps")
            set(_conanfile_path "${CMAKE_SOURCE_DIR}/conanfile.txt")
        endif()
        get_property(_multiconfig_generator GLOBAL PROPERTY GENERATOR_IS_MULTI_CONFIG)
        if(NOT _multiconfig_generator)
            message(STATUS "CMake-Conan: Installing single configuration ${CMAKE_BUILD_TYPE}")
            conan_install(${_conanfile_path} ${_host_profile_flags} ${_build_profile_flags} --build=missing ${generator})
        else()
            message(STATUS "CMake-Conan: Installing both Debug and Release")
            conan_install(${_conanfile_path} ${_host_profile_flags} ${_build_profile_flags} -s build_type=Release --build=missing ${generator})
            conan_install(${_conanfile_path} ${_host_profile_flags} ${_build_profile_flags} -s build_type=Debug --build=missing ${generator})
        endif()
        unset(_host_profile_flags)
        unset(_build_profile_flags)
        unset(_multiconfig_generator)
        unset(_conan_install_success)
        unset(_conanfile_path)
    else()
